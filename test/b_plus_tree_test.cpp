...
TEST(BPlusTreeIteratorTest, HandlesSimpleIteration) {
    // 1. 准备阶段：创建一个B+Tree实例，并乱序插入几个元素。
    // 使用 <std::string, int> 来验证模板的通用性。
    // BPlusTree的构造函数有默认的 std::less<KeyT> 比较器，
    // 它能正确处理 std::string 的比较。

    bptree::BPlusTree<std::string, int> tree(3, 3);
    tree.Insert("cat", 100);
    tree.Insert("dog", 200);
    tree.Insert("apple", 50);

    // 2. 期望结果：定义一个按键排序的期望结果向量。
    std::vector<std::pair<std::string, int>> expected_result = {
            {"apple", 50},
            {"cat",   100},
            {"dog",   200}
    };

    // 3. 执行与断言：使用范围for循环遍历B+Tree。
    // 这行代码会被编译器展开，并调用 tree.begin() 和 tree.end()。
    // 在每次循环中，它会调用迭代器的 operator* 来初始化 `pair`。
    int i = 0;
    for (const auto &pair: tree) {
        // 在循环内部，`pair` 是一个 `const std::pair<const std::string, int>&` 类型。
        // 它引用了 operator* 返回的临时对象。ú

        // 检查当前遍历到的键和值是否与期望结果匹配。
        ASSERT_LT(i, expected_result.size()); // 确保不会越界访问
