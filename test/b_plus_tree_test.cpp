...
TEST(BPlusTreeInsertionTest, HandlesSimpleInsertAndGet) {
    ...
    // 断言1：成功插入一个新键
    bptree::Transaction txn1;
    EXPECT_TRUE(tree.Insert(10, 100, &txn1));
    ...
    EXPECT_FALSE(tree.Insert(10, 200, &txn1));
...
TEST(BPlusTreeInsertionTest, HandlesLeafNodeSplit) {
    ...
    bptree::Transaction txn2;
    tree.Insert(10, 100, &txn2);
    tree.Insert(20, 200, &txn2);
    tree.Insert(15, 150, &txn2); // 触发分裂
...
TEST(BPlusTreeInsertionTest, HandlesInternalNodeSplit) {
    ...
    bptree::Transaction txn3;
    tree.Insert(10, 100, &txn3);
    tree.Insert(20, 200, &txn3);
    tree.Insert(30, 300, &txn3);

    tree.Insert(15, 150, &txn3);
    tree.Insert(40, 400, &txn3);

    tree.Insert(50, 500, &txn3);
    tree.Insert(35, 350, &txn3);
...
TEST(BPlusTreeInsertionTest, HandlesLargeNumberOfRandomInsertions) {
    ...
    for (int key: keys) {
        ...
        ASSERT_TRUE(tree.Insert(key, key * 10, &txn)); // 使用ASSERT确保插入不会意外失败
    }
...
TEST(BPlusTreeIteratorTest, HandlesSimpleIteration) {
    ...
    {
        bptree::Transaction txn;
        tree.Insert(2, 100, &txn);
        tree.Insert(3, 200, &txn);
        tree.Insert(1, 50, &txn);
    }
...
TEST(BPlusTreeIteratorTest, HandlesIterationAcrossNodeSplits) {
    ...
    bptree::Transaction tra;
    for (int key: keys) {
        tree.Insert(key, key * 10, &tra);
    }
...
TEST(BPlusTreeRemoveTest, HandlesRemoveWithRedistributionFromRight) {
    ...
    tree.Insert(10, 100,&tra);
    tree.Insert(20, 200,&tra);
    tree.Insert(30, 300,&tra);
    tree.Insert(40, 400,&tra);
    tree.Insert(50, 500,&tra);
...
TEST(BPlusTreeRemoveTest, HandlesRemoveWithRedistributionFromLeft) {
    ...
    // 结构: 根[30] -> 左叶[10, 20, 25], 右叶[30, 40]
    {
        bptree::Transaction txn;
        tree.Insert(20, 200, &txn);
        tree.Insert(25, 250, &txn);
        tree.Insert(30, 300, &txn);
        tree.Insert(40, 400, &txn);
        tree.Insert(10, 100, &txn);
    }
